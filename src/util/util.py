...
class Data:
	def __init__(self):
		...
		self.coords = [(self.lon[self.row[i]][self.col[i]], self.lat[self.row[i]][self.col[i]], i) for i in range(4732) ]
...
def get_children(root):
	children = [ root[2][1][1][i] for i in range(4732) ]
...
def write_ids(nodes):
	for i in range(4732):
		nodes[i].attrib["style"] = "fill:#f7fcfd;"
